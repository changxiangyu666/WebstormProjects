...
$.loadmore.get(getData, {
    click: false, //默认是true，是否支持点击加载更多
    scroll: true, //默认是false，是否支持滚动加载
    size: 8, //每页显示个数，默认是10
    url: '??', //数据url，不能为空
    code: {'??':'??'}, //url参数
    desc: 'time', //结果集排序字段，不能为空
    divID: 'panel-div',//内容承载div id，默认为div-load-more
//            btnID: '',//加载更多按钮id，默认为btn-load-more
...
function SucceedFun(data) {
    if (!isNull(data)) {
        var contents = {
            "data": data,
            statusRander: function () {
                if (this.accept_status == '0') {
                    return '<a  class=\'btn-danger btn-sm pull-right\' >未受理</a>';
                } else {
                    return '<a class=\'btn btn-success btn-sm pull-right\' data-toggle="modal" data-target="#myModal2" onclick=\'ssaRdingdanQuery(' + this.num + ');\'>详  细</a>';
                }
            }
        };
        $wrap.append(Mustache.to_html(tpl, contents));
    }
}